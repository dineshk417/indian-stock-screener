...
def _type_col(df: pd.DataFrame, col: str = "Type") -> pd.DataFrame:
    ...
    if txn_filter == "All" or col not in df.columns:
        return df
    return df[df[col].fillna("").astype(str).str.upper().str.contains(txn_filter.upper(), na=False)]
...
with tab_deals:
    ...
    if frames:
        ...
        if "Deal" in combined.columns:
            combined["Deal"] = combined["Deal"].fillna("").astype(str).str.title()
    ...
    # ── Stats strip ────────────────────────────────────────────────────────────────────
    if not combined.empty:
        _n_bulk    = int((combined["Deal"].fillna("").astype(str) == "Bulk").sum())
        _n_block   = int((combined["Deal"].fillna("").astype(str) == "Block").sum())
        _n_buy     = int(combined["Type"].fillna("").astype(str).str.upper().str.contains("BUY", na=False).sum())
...
with tab_flow:
    ...
    if flow_df.empty:
        ...
        _fii = flow_df[flow_df["Category"].fillna("").astype(str).str.upper().str.contains("FII|FPI", na=False)] if "Category" in flow_df.columns else pd.DataFrame()
        _dii = flow_df[flow_df["Category"].fillna("").astype(str).str.upper().str.contains("DII", na=False)] if "Category" in flow_df.columns else pd.DataFrame()
...
with tab_insider:
    ...
    if insider_df.empty:
        ...
        _n_buy  = int(insider_df["Txn"].fillna("").astype(str).str.upper().str.contains("BUY|ACQUI", na=False).sum()) if "Txn" in insider_df.columns else 0
        ...
        if "Disclosed" in insider_df.columns and "Txn" in insider_df.columns:
            ...
            _ins_ts["_is_buy"] = _ins_ts["Txn"].fillna("").astype(str).str.upper().str.contains("BUY|ACQUI|CREAT", na=False)
