"""
Sentiment analysis with three engines in priority order:
  1. Claude (Anthropic) — if ANTHROPIC_API_KEY is set
  2. Groq               — if GROQ_API_KEY is set (free, contextual, changes daily)
  3. VADER              — free, local, deterministic fallback

All engines return the same structured dict so the rest of the app is unaffected.
"""
import json
import os
import re
...
def _groq_market_sentiment(news_text: str) -> Optional[dict]:
    """Use Groq (free) for contextual market sentiment analysis."""
    try:
        ...
        logger.error(f"Failed to parse Groq sentiment response as JSON: {e}")
        return None
...
def analyze_market_sentiment(news_text: str, use_cache: bool = True,
    ...
    Priority: Claude → Groq → VADER.
    Cache is scoped to today's date so the score refreshes each calendar day.
    """
    cache = get_cache()
    # Include today's date so sentiment refreshes every calendar day
    cache_key = f"sentiment:market:{date.today().isoformat()}"
    ...
    # ── Groq path (free, contextual — preferred over VADER) ───────────────────
    result = _groq_market_sentiment(news_text)
    if result is not None:
        if use_cache:
            cache.set(cache_key, result, CACHE_TTL_SENTIMENT)
        return result

    # ── VADER fallback (deterministic, always available) ──────────────────────
    items = news_items or []
...
def has_api_key() -> bool:
    return bool(os.getenv("ANTHROPIC_API_KEY") or os.getenv("GROQ_API_KEY"))
